...
class AppStore extends Component {
  ...
  getFilteredTabs = searchResults => {
    const {activeTabId} = this.state
    const filterAppItems = searchResults.filter(
  ...
  render() {
    ...
    const filteredApps = this.getFilteredTabs(searchResults)
    ...
    if (filteredApps.length === 0) {
      return (
        <div className="app-container">
          ...
          <h1 className="alert"> No Apps found</h1>
        </div>
      )
    }
